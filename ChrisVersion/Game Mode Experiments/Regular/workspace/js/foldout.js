...
function scaleDifficulty(){
	...
	for(var i = 0; i < faceArray.length; i++) {
		//Set all non pivots to their set values.
		if(faceArray[i].trueColor !== 'black'){
			//Sides that are not the answer 
			if( i !== answer){
				faceArray[i].value = faceArray[i].trueValue;
				faceArray[i].trueColor = 'white';
			}
...
function foldoutT(foldoutNum){
	...
	for(var i = 0; i <foldoutArray[foldoutNum].length; i++){
		if(foldoutArray[foldoutNum][i]){
			...
			//Changes 2d Foldout to display no image if color = white.
			if(foldoutArray[foldoutNum][i].playerColor === 'white'){
				$('#' + foldoutArray[foldoutNum][i].id + ' img').css('display','none');
			}
...
function rotateFace(id){
	...
	//no rotation on white faces for simplicity.
	if(currentFace.playerColor != 'white'){
...
function applyFaces(foldoutNum){
	...
	for(var i = 0; i < faces; i++){
		...
		if(faceArray[i].trueColor !== 'white'){
			$('#cube .' + faceNames[i].substring(4) + ' img').css('display','block');
			$('#cube .' + faceNames[i].substring(4)).css('background-color',faceArray[i].trueColor);
		} else {
			$('#cube .' + faceNames[i].substring(4) + ' img').css('display','none');
			$('#cube .' + faceNames[i].substring(4)).css('background-color',faceArray[i].trueColor);
		}
	}
}
